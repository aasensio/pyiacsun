...
!-----------------------------------------------------------------	
	...
	type(dual) :: zeeman_voigt(:,:), zeeman_faraday(:,:)
	integer :: n, nlow, nup, iup, ilow, i_pi, i_blue, i_red, cual, i, j
	real(kind=8) :: Mup, Mlow, strength, splitting
	type(dual) :: va, vb
		...
		do i = 1, 3
			do j = 1, line%nLambda
				call zeeman_voigt(i,j)%init(0.d0)
				call zeeman_faraday(i,j)%init(0.d0)
			enddo
		enddo
		...
		do iup = 1, nup
			...
			do ilow = 1, 3
				...
				if (abs(Mlow) <= linea%Jlow) then
					...
					profile = voigtDual(model%damping,v-va+vb*splitting)
...
!-----------------------------------------------------------------	
	...
	integer :: line
	type(dual) :: zeeman_voigt(:,:), zeeman_faraday(:,:)
	type(dual) :: ki(:), kq(:), ku(:), kv(:), fq(:), fu(:), fv(:)
	type(dual) :: sin_theta, cos_theta, sin_2chi, cos_2chi
