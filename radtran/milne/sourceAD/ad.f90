...
implicit none
	...
	type, public :: dual	
		real(kind=8) :: rp = 0.d0
		real(kind=8), dimension(9) :: ip = (/0.d0,0.d0,0.d0,0.d0,0.d0,0.d0,0.d0,0.d0,0d0/)   ! Real and infinitesimal part
	contains
		procedure, pass :: init
	end type dual
...
! Operator overload
	...
	! interface operator(=)
		! module procedure equalDualReal, equalDualRealVector
	! end interface
...
! EQUALITY
!----------------
	! elemental function equalDualReal(a) result(c)
	! 	type (dual), intent(in) :: a
	! 	type (dual) :: c
	! 	c%rp = a%rp
	! 	c%ip = b%rp
	! end function equalDualReal
