...
!------------------------------------------------
	...
	integer :: i, loop
		...
		step = (atmosphere%lTau500(2) - atmosphere%lTau500(1))
		...
		do i = 1, 10
			call partitionAtomic(atmosphere%T, electronRelease(i), u1, u2, u3, ei1, ei2, weight, abundances(i))			
			phi(i,:) = saha(atmosphere%T, one, u1, u2, ei1)
			tdphi(i,:) = saha(1.001d0*atmosphere%T, one, u1, u2, ei1)
		enddo		
		
		tdphi = (tdphi - phi) / 0.001d0
		abundancesRatio = abundances / 1.10165d0
						
		call deriv(atmosphere%lTau500,log10(atmosphere%T),dtdx)			
...
! Integrate the hydrostatic equilibrium equation
		...
		atmosphere%Pe(1) = sqrt(GRAV * 10.d0**atmosphere%lTau500(1) / k_c * pe0)		
		...
		do while (delta > 0.1d0)
			...
			dy(1) = (GRAV * 10.d0**atmosphere%lTau500(1)/k_c+dpdgt)/der
			pe0 = atmosphere%Pe(1)
			if (dy(1) <= 0.d0) then
		...
		do i = 2, atmosphere%nDepths
			y(i) = y(i-1) + (atmosphere%lTau500(i) - atmosphere%lTau500(i-1)) * dy(i-1)
			...
			dy(i) = (GRAV * 10.d0**atmosphere%lTau500(i)/k_c+dpdgt)/der
		enddo
				
	end subroutine hydrostaticEquilibrium
