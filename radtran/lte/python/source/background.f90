...
module backgroundOpacityModule
use constants_mod
use general_routines_mod
use mathsModule, only : gasPressure
implicit none
contains
...
!-----------------------------------------------------------------
	function backgroundOpacity(T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda_in)
	real(kind=8) :: backgroundOpacity, T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda_in, PTotal
...
!   Mg bound-free + free-free
!-----------------------------------------------------------------
	function backgroundOpacityPerHParticle(T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda_in)
	real(kind=8) :: backgroundOpacityPerHParticle, T, Pe, PH, PHminus, PHplus, PH2, PH2plus, lambda_in, PTotal
	real(kind=8) :: factor, H_per_volume
	real(kind=8) :: contrib1, contrib2, contrib3, contrib4, contrib5, contrib6, contrib7	
			
		factor = 1.d0 + (PHminus + PHplus + 2.d0*(PH2+PH2plus)) / PH
		
		contrib1 = hminus_ff(T, Pe, lambda_in) / factor
		contrib2 = hminus_bf(T, Pe, lambda_in) / factor
		contrib3 = hydrogen(T, lambda_in) / factor
		contrib4 = thomson(Pe, PH) / factor
		contrib5 = rayleigh_h(lambda_in) / factor
 		contrib6 = rayleigh_h2(PH, PH2, lambda_in) / factor
		
! Magnesium opacity is per total H atom. For this reason, we multiply by factor
! because nH*factor=nH_tot
		contrib7 = magnesium(T, Pe, lambda_in)
						
		backgroundOpacityPerHParticle = (contrib1 + contrib2 + contrib3 + contrib4 + contrib5 + contrib6 + contrib7)		
				
	end function backgroundOpacityPerHParticle	

	
end module backgroundOpacityModule