...
cdef extern:
	...
	void c_synthlines(int *nDepths, double *atmosphereIn, int *nLambda, double *stokesOut, double *dStokesOut)
...
def synthLines(ar[double,ndim=2] atmosphereIn):
	...
	cdef:
		...
		ar[double, ndim=3] dStokesOut = empty((5,nLambdaGlobal,nDepths), order='F')
	
	# Make sure that the 2D array is C_CONTIGUOUS
	atmosphere = ascontiguousarray(atmosphereIn)	
	c_synthlines(&nDepths, &atmosphere[0,0], &nLambda, <double*> stokesOut.data, <double*> dStokesOut.data)
	
	return stokesOut[0:4,:], stokesOut[4,:], dStokesOut