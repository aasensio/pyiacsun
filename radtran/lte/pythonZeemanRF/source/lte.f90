module lteMod
use globalModule, only : atmosphere, lineList, PH, PC, PK, PHK, UMA, EV_ERG, SQRTPI, PI, identity, dualSize
...
! 		write(*,*) 'Number of active lines : ', activeLines
	...
	subroutine c_synthlines(nDepths, atmosphereIn, nLambda, stokesOut, dStokesOut) bind(c)
	integer(c_int), intent(in) :: nDepths, nLambda
	real(c_double), intent(in) :: atmosphereIn(7,nDepths)
	type(dual) :: stokesDual(5,nLambda)
	real(c_double), intent(inout) :: stokesOut(5,nLambda)
	real(c_double), intent(inout) :: dStokesOut(5,nLambda,nDepths)
	integer :: i
		...
		do i = 1, nDepths
			call atmosphere%vmac(i)%init(atmosphereIn(4,i), i)
		enddo
...
! Synthesize lines				
		call synthLines(atmosphere, stokesDual)

		stokesOut = stokesDual%rp
		do i = 1, nDepths
			dStokesOut(:,:,i) = stokesDual%ip(i)
		enddo
