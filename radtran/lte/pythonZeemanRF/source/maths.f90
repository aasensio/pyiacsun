...
! ---------------------------------------------------------
   ...
   real*8 :: temporal(size(T))
		...
		where (T /= 0.d0)
			planckFrequencyVector = PHC * nu**3.d0 / (exp(PHK * nu / T) - 1.d0)
		elsewhere
...
! ---------------------------------------------------------
   ...
   real*8 :: temporal
      if (T /= 0.d0) then
         planckFrequencyScalar = PHC * nu**3.d0 / (exp(PHK * nu / T) - 1.d0)
      else
...
! ---------------------------------------------------------
   ...
   real*8 :: temporal(size(T))
		...
		where (T /= 0.d0)
			planckWavelengthVector = PHC2 / lambda**5.d0 / (exp(PHK * PC / (lambda * T)) - 1.d0)
		elsewhere
...
! ---------------------------------------------------------
   ...
   real*8 :: temporal
      ...
      if (T /= 0.d0) then
         temporal = exp(PHK * PC / (lambda * T) - 1.d0)
...
!--------------------------------------------------------------
	...
	real(kind=8) :: da(:)
	real(kind=8) :: dv(:), fvoigt_zeeman(size(dv),2)
	complex :: w4, z, t, u, v4
	real*8 :: s
	integer :: i, n
		...
		do i = 1, n
			...
			fvoigt_zeeman(i,1) = dble(w4)
			fvoigt_zeeman(i,2) = aimag(w4)
...
! ---------------------------------------------------------
   ...
   type(dual) :: short_car_parab
   type(dual), INTENT(IN) :: dtm, dtp
	type(dual), INTENT(INOUT) :: psim, psi0, psip
   type(dual) :: exu, u0, u1 ,u2, d2, d3, d4
...
! ---------------------------------------------------------
   ...
   type(dual) :: short_car_linea
   type(dual), INTENT(IN) :: dtm
	type(dual), INTENT(INOUT) :: psim, psi0
  	type(dual) :: exu, u0, u1, c0, cm, d2
...
!--------------------------------------------------------------
	subroutine invert(a)
	type(dual) :: a(4,4)
	type(dual) :: b(4,4), det, maxim, fabsmax
	integer :: i, j
...
! ---------------------------------------------------------
	...
	type(dual) :: A(4,4), B(4,4)
	type(dual) :: productMatrix(4,4)
	integer :: i, j, k
		...
		do i = 1, 4
			...
			enddo
		enddo
	end function productMatrix

! ---------------------------------------------------------
! Product matrix
! ---------------------------------------------------------
	function productMatVector(A, B)
	type(dual) :: A(4,4), B(4)
	type(dual) :: productMatVector(4)
	integer :: i, k

		productMatVector = 0.d0
		do i = 1, 4
			do k = 1, 4
				productMatVector(i) = productMatVector(i) + A(i,k) * B(k)
			enddo		
		enddo
	end function productMatVector
...
! ---------------------------------------------------------
	...
	type(dual) :: opacity(:,:)
	real(kind=8) :: height(:), source(:), mu, boundary(:)
	type(dual) :: formal_sol_polarized(4), Inten(4)
	integer :: k, km, kp, idir, k0, kf
	type(dual) :: chim, chi0, chip, dtp, dtm, exu
	type(dual) :: psim, psi0, psip, psim_lin, psi0_lin, dm, dp

	integer :: i, j, n
	type(dual), allocatable :: ab_matrix(:,:,:), source_vector(:,:)
	type(dual) :: sm(4), s0(4), sp(4), mat1(4,4), mat2(4,4), mat3(4,4), Inten2(4)
...
! Parabolic short-characteristics
			if (k /= kf) then
				...
				s0 = source_vector(:,k)
				sp = source_vector(:,kp)
				dm = abs(1.d0/mu*(height(k) - height(km)))
				dp = abs(1.d0/mu*(height(kp) - height(k)))
			else
! Linear short-characteristics
				km = k - idir
				chim = opacity(1,km)
			...
			Inten2 = productMatVector(mat1, Inten)

			if (k /= kf) then
				call par_sc(dtm,dtp,psim,psi0,psip)				
				Inten = productMatVector(mat2, Inten2 + psim*sm + psi0*s0 + psip*sp)
				! Inten = matmul(mat2,matmul(mat1,Inten) + psim*sm + psi0*s0 + psip*sp)
			else
				call lin_sc(dtm,psim,psi0)				
				Inten = productMatVector(mat2, Inten2 + psim*sm + psi0*s0)
...
!----------------------------------------------------------------				
	function voigtDual(a, v)
		type (dual) :: a(:), v(:)
		type (dual) :: voigtDual(size(v),2)
		integer :: ierr, i
		real(kind=8) :: H(size(v),2), ga(size(v)), gv(size(v)), PI = 3.14159265359d0
			...
			do i = 1, size(v)
				voigtDual(i,1)%ip = gv(i) * v(i)%ip + ga(i) * a(i)%ip
				voigtDual(i,2)%ip = -ga(i) * v(i)%ip + gv(i) * a(i)%ip
			enddo			
