...
use mathsModule, only : saha, calculateDamping, planckFrequency, shortCharacteristics, computeHeight, gasPressure, &
	strength_zeeman, formal_sol_polarized, voigtDual
	...
	real(kind=8) :: Mup, Mlow, strength
		...
		do ilow = 1, 3
			do cual = 1, transition%nComponents(ilow)
				...
				atmos%profile = voigtDual(transition%damping, (transition%frequency0 - frequency - transition%frequency0 * 1e5 * atmos%vmac / PC + atmos%splitting) / transition%deltaNu)
