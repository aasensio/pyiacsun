...
ext_modules = [Extension(# module name:
                         "pymilne_{0}".format(uname),
                         # source file:
                         ['pymilne.pyx'],
                         # other compile args for gcc
                         extra_compile_args=['-fPIC', '-O3'],
                         # other files to link to
                         extra_link_args=['vars.o', 'maths.o', 'atomic.o', 'milne.o','-lgfortran'])]

setup(name = 'pymilne_{0}'.format(uname),
...
system('cp pymilne_{0}.so ../'.format(uname))
