...
class b0(object):
    ...
    w : array, shape = (n_features)
        sparse/block sparse weight vectors
    ...
    def fit_transform(self, X, y, blk_start_loc=None):
        ...
        for count in range(self.max_iters):
            ...
            if (count >= self.max_iters):
                break
            if (self.verbose):
                print("Iters: {0:3d} - Num. coeffs: {1:3d} - minGamma: {2:12.8f} - gammaChange: {3:12.8f} - muChange: {4:12.8f}".format(count, 
                    len(index), np.min(gamma), np.max(np.abs(gamma-gamma_old)), dmu))
...
# Generate nonzero block coefficients
    ...
    for i in range(1,blkLen):
        blks[:,i] = beta * blks[:,i-1] + np.sqrt(1.0-beta**2) * np.random.randn(blkNum)
    ...
    clf = b0(learn_lambda=0, learn_type=1, lambda_init=1e-3, 
              epsilon=1e-5, max_iters=100, verbose=1)

    out = clf.fit_transform(Phi, Y, groupStartLoc)
